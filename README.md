...
대시보드: http://localhost:8433 — **여기서 전부 가능하다**:
실시간 토큰 스트리밍(에이전트가 일하는 걸 그대로 지켜봄) · 팔로우업(같은 세션에 추가 지시,
Enter로 전송) · 결재(윤허/불허) · 실패 노드 재시도 · 취소/보관 · git 변경사항(diff) 뷰 ·
지시 내용 열람 · 역할(신하) 편집기 · 반복 어명 관리 · ⌘K 팔레트 · ⌘N 새 어명.
