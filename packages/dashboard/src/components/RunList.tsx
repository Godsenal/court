...
const GROUPS: Array<{ id: string; label: string; match: (r: RunSummary) => boolean }> = [
  ...
  { id: "running", label: "실행 중", match: (r) => r.status === "running" },
  { id: "done", label: "완료됨", match: (r) => !["waiting_human", "running"].includes(r.status) },
];
    ...
    <aside className="flex w-80 shrink-0 flex-col border-r border-line bg-panel">
      <header className="border-b border-line p-3">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h1 className="text-[15px] font-semibold tracking-tight">Court</h1>
            <p className="text-[11px] text-faint">AI orchestration</p>
          </div>
          <button
            onClick={onCompose}
            className="rounded-lg bg-accent px-3 py-1.5 text-[13px] font-semibold text-ink transition hover:brightness-110"
          >
            ＋ 새 작업
          </button>
        </div>
        <input
          ...
          placeholder="작업 검색…"
          className="mt-2.5 w-full rounded-lg border border-line bg-ink/60 px-3 py-1.5 text-[13px] outline-none placeholder:text-faint focus:border-accent/50"
        />
      </header>
      <div className="flex-1 overflow-y-auto pb-4">
        ...
        {!filtered.length && (
          <p className="px-4 py-8 text-center text-[13px] text-faint">
            {query ? `"${query}"에 맞는 작업이 없습니다` : "아직 어명이 없습니다 — ＋ 새 작업으로 시작"}
          </p>
        )}
      </div>
    </aside>
  ...
  return (
    <button
      ...
      className={`block w-full px-4 py-2.5 text-left transition ${
        active ? "bg-panel-3 shadow-[inset_2px_0_0_var(--color-accent)]" : "hover:bg-panel-2"
      }`}
    >
      ...
      <div className="mt-1.5 flex items-center gap-2 pl-4">
        <span className={`h-[3px] flex-1 overflow-hidden rounded-full ${run.status === "running" ? "shimmer-line" : "bg-line"}`}>
          <span
            className={`block h-full rounded-full transition-all ${run.status === "failed" ? "bg-ruby/70" : "bg-accent/80"}`}
            style={{ width: `${pct}%` }}
          />
        </span>
        ...
        {run.waiting.length > 0 && <span className="size-1.5 rounded-full bg-amber" />}
      </div>
    </button>
  );
}
